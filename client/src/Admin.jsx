...
import { uploadToBlob } from './api/uploads'
...
export default function Admin({ onNavigate, onLogout }) {
  ...
  const selectBoss = slug => { setForm(toForm(bosses.find(boss => boss.slug === slug))); setUploadStatus('') }
  const chooseImage = async event => {
    ...
    try {
      const pathname = await uploadToBlob(file, { folder: 'bosses', token, onProgress: percent => setUploadStatus(`Uploading ${percent}%`) })
      setForm(current => ({ ...current, imagePath: pathname, imagePreview: URL.createObjectURL(file) }))
      setUploadStatus('Image uploaded. Save changes to apply it.')
    } catch (error) { setUploadStatus(error.message || 'Upload failed') }
  }
  ...
  const save = async event => {
    ...
    if (payload.imagePath) delete payload.imageUrl; else delete payload.imagePath
    ...
    try {
      ...
      setForm(toForm(updated)); setUploadStatus(''); setStatus('Saved to server.')
  ...
  return <main className="min-h-screen bg-black px-5 py-10 text-white sm:px-10">
    <div className="mx-auto max-w-5xl">
      ...
      <section className="mb-6 flex flex-wrap items-center gap-5 border border-white/20 p-5">{form.imagePreview || form.imageUrl ? <img src={form.imagePreview || form.imageUrl} alt={`${form.name} artwork`} className="h-28 w-28 border border-white/20 object-cover" /> : <div className="flex h-28 w-28 items-center justify-center border border-dashed border-white/25 text-center text-[10px] uppercase tracking-[.12em] text-white/40">Built-in artwork</div>}<div><p className="text-xs font-bold tracking-[.25em] text-[#e0ff38]">BOSS IMAGE</p><label className="mt-3 inline-block cursor-pointer border border-white/30 px-4 py-2 text-xs font-bold uppercase tracking-[.16em] hover:border-[#e0ff38]">Upload image<input type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/avif" onChange={chooseImage} className="hidden" /></label><p className="mt-2 text-sm text-white/55">{uploadStatus || 'JPEG, PNG, WebP, GIF or AVIF, up to 10 MB.'}</p></div></section>
